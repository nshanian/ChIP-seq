...
This repository contains workflows and tools for performing alignment and peak calling on paired-end (PE) histone ChIP-seq data in `.fastq` file format generated by next-generation sequencing (NGS).

The workflow is written for acetyl-lysine (Kac) and propionyl-lysine (Kpr) histone ChIP-seq on H3K18 and H4K12 using two replicates per histone mark. 
...
Aligned sequence results and peak files in `.bam` and `.narrowPeak` format can be used to perform downstream statistical analyses such as differential binding, annotation, TSS distribution, as well as pathway and motif enrichment analysis.
